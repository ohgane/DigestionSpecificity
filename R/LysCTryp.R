#' Peptide identification results of HeLa cells digested with LysC and Trypsin
#'
#' Identification results from a bottom-up proteomics experiment. HeLa cell lysate was digested with LysC, and Trypsin using TFE digestion protocol on an AmiconUltra 10K, and the digest was analyzed on a Thermo LTQ-Orbitrap XL in data-dependent acquisition mode (top 3, orbitrap detector for MS1 at 60000 resolution, ion trap for MS2, and CID fragmentation with 35% nCE). The obtained data was searched against a subset of human protein database (from uniprot, ~1500 sequences) by using SearchGUI (X!Tandem and OMSSA, unspecific search, precursor tolerance 4 ppm, and fragment ion tolerance 0.4 Da). The identification results were processed with PeptideShaker (FDR 1%) and exported to xls file (as "Default Peptide Report").
#'
#' @docType data
#'
#' @usage data(LysCTryp)
